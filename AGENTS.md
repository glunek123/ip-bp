...
- 加载下节运行环境并执行`pnpm context:check`。漂移时核对真实改动和最近提交，不直接刷新快照、回退代码或套用旧验证。
...
- Skills按问题触发：复杂设计用设计类，真实故障用调试类，浏览器验证用测试类，高风险验收用验证类；普通开发不叠加方法论Skills。默认单Agent，只有用户要求、独立模块可并行、大范围审查或高风险发布前Review才使用子Agent。
...
- 开发循环运行直接相关测试；快速静态门禁使用`pnpm check:fast`。`pnpm verify`用于Level 3、完整业务切片、合并`main`前、核心架构变更或跨模块回归怀疑；E2E和数据库专项只在风险触发时运行。
...
- 合并并核实远端后清理任务worktree和已合并本地分支；脏worktree先判断和保全，禁止强删。具体步骤见三级规则。
