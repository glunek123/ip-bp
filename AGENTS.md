...
- 开工与收口更新开发状态；暂停或阻断时补充恢复所需检查点。同次交付同步受影响文档、实际验证和未决项。按 [AI 开发规范](docs/ai-coding.md)完成验证、回填结果、最终快照和差异核对；验证后再改代码或配置时重新验证，仅回填非执行性文档时检查格式与快照。记录快照不等于测试通过，测试通过不等于获准上线。
...
- 每项任务的风险分级、候选验证、证据复用、独立审查与自动版本管理统一按[风险分级与已验证集成规则](.cursor/rules/verified-feature-integration.mdc)执行；达到门禁时不重复询问合并与推送。
