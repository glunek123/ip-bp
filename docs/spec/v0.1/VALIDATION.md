...
独立测试库`dev_cor_test`的数据库型Playwright新增3项并通过：管理员从正式页面创建第二位人员后，数据库存在账号、散列凭据、当前部门成员、Team范围角色和创建审计，新人员可用真实密码登录；活动Team角色阻止直接调队，页面提示先撤权，撤权后可直接调队；管理员只填写目标新密码即可重置，旧密码随后被拒绝、新密码可登录。验收人员不是由测试夹具直接插入，夹具只准备首位管理员、两个Team和两个角色模板。
