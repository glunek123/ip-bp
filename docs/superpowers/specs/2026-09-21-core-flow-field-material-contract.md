...
业务Command在自己的Serializable事务中通过材料层公共边界处理版本。`assertAvailableVersions(transaction, actor, input)`重用当前CUSTOMER／LEAD_DRAFT／LEAD授权，并且只用caller传入的Prisma transaction client读取当前授权快照和校验部门、owner、category、Material `ACTIVE`与ContentVersion `AVAILABLE`；重复或任一缺失／越界版本都整体拒绝。返回值按输入ID顺序固定，是包含`materialId/contentVersionId/purpose/mimeType/ownerType/ownerId`的不可变validated facts，其中purpose只来自稳定Material。`freezeReferences(transaction, input)`只接受上述validated facts，不接受caller覆盖purpose或未验证的裸contentVersionId，并使用同一caller transaction批量冻结精确`materialId/contentVersionId`引用；已存在唯一引用按幂等跳过。validated facts通过服务内私有WeakMap以对象身份登记并绑定精确caller transaction，prototype派生、复制、JSON往返和跨事务复用都拒绝；写引用只读WeakMap中的canonical fact。

当前Lead截图集合另由材料层管理，Lead模块不得直接查写`MaterialReference`。`listCurrentReferenceVersionIds(reader, actor, input)`只支持`resourceType=lead`、`purpose=LEAD_SCREENSHOT`，在caller reader中重查`lead.read`后只返回`actionEventId=null`的去重稳定排序版本ID。`replaceCurrentReferences(transaction, actor, input)`在同caller transaction中重查`lead.edit`，只接受同tx canonical且owner为目标Lead的`LEAD_SCREENSHOT`事实，仅删除并重建null-action当前引用，从不返回或删除带action event的历史引用。返回的`beforeVersionIds/afterVersionIds`均按集合语义去重排序，`changed`只比较集合内容，不把顺序变化视为业务变化。上述公共边界均不得回落全局`DatabaseService`。
...
- 当前客户准入能力尚未正式部署，正常升级不应存在旧准入回执。若迁移异常发现旧回执，仅当 Customer 当前版本与回执结果版本完全相等时才允许从当前态构造可证明快照；版本或关联不符必须原子阻断迁移，由运维依据审计证据人工恢复或清理，禁止把当前字段与历史版本号拼接成伪历史响应。
