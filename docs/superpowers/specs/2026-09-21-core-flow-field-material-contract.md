...
业务Command在自己的Serializable事务中通过材料层的两个公共边界处理版本。`assertAvailableVersions(transaction, actor, input)`重用当前CUSTOMER／LEAD_DRAFT授权，并且只用caller传入的Prisma transaction client读取当前授权快照和校验部门、owner、category、Material `ACTIVE`与ContentVersion `AVAILABLE`；重复或任一缺失／越界版本都整体拒绝。返回值按输入ID顺序固定，是包含`materialId/contentVersionId/purpose/mimeType/ownerType/ownerId`的不可变validated facts，其中purpose只来自稳定Material。`freezeReferences(transaction, input)`只接受上述validated facts，不接受caller覆盖purpose或未验证的裸contentVersionId，并使用同一caller transaction批量冻结精确`materialId/contentVersionId`引用；已存在唯一引用按幂等跳过。validated facts通过服务内私有WeakMap以对象身份登记并绑定精确caller transaction，prototype派生、复制、JSON往返和跨事务复用都拒绝；写引用只读WeakMap中的canonical fact。Command不得复制材料查询，两个方法也不得回落全局`DatabaseService`。
