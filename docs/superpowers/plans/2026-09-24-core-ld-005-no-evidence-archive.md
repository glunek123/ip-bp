...
- [ ] 集成：运行聚焦测试、`pnpm check:fast`，检查现有审核／撤回路径不回归；高风险任务独立审查及集成候选 Final Review 后固定候选。
- [ ] 正式验证：按三级规则在独立测试库运行迁移空库和上一支持 Schema 升级、真实数据库 API 反例、运营与企业客户双账号浏览器链（刷新后仍归档）。同步 Spec／状态／VALIDATION；未通过前不标记完成或推进 Current。
