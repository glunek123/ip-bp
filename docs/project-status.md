...
**CORE-LD-002｜运营推送至真实客户端审核（待正式领取）**：在现有待推送线索上补运营推送动作、企业客户账号绑定和最小真实客户端；推送后状态进入`WAITING_REVIEW`，绑定账号只能读取本企业获推送线索。本Slice不实现侵权结论、不侵权归档、批量推送或完整客户门户。唯一Current／Next及后续顺序以[功能开发路线图](feature-roadmap.md)为准。
...
正式领取CORE-LD-002后，先把企业账号绑定、运营推送Command和客户端企业范围收口为同一最小实施计划，再开始数据库／后端／页面实现。本次状态维护不等于领取或实现该Slice；不自动推送、发布或操作生产数据库。
